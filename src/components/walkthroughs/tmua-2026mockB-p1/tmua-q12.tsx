...
const META = { questionNumber: 12, set: "B", paperNumber: "Paper 1", topicTag: "Quadratics / Optimisation" };
...
const SECTIONS_Q12 = [
  { type: "prose", text: (<>A family of quadratic curves is given by</>) },
  { type: "mathbox", tex: "y_k = (x - k)^2 + 2k^2 - 4k + 5" },
  { type: "prose", text: (<>where <Tex>{"k"}</Tex> is any real number.</>) },
  { type: "prose", text: (<>All these curves are sketched, and the point with the lowest y-coordinate among all the curves is <Tex>{"(a, b)"}</Tex>.</>) },
  { type: "prose", text: (<>Find the value of <Tex>{"a + b"}</Tex>.</>) },
];
const OPTIONS_Q12 = [["A", "-1"], ["B", "0"], ["C", "1"], ["D", "3"], ["E", "4"], ["F", "7"]];
