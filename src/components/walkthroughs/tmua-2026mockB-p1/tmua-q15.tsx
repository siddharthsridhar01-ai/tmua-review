...
const META = { questionNumber: 15, set: "B", paperNumber: "Paper 1", topicTag: "Exponentials / Trig" };
...
const SECTIONS_Q15 = [
  { type: "prose", text: (<>The difference between the maximum and minimum values of the function</>) },
  { type: "mathbox", tex: "f(x) = a^{\\cos x}" },
  { type: "prose", text: (<>where <Tex>{"a > 0"}</Tex> and <Tex>{"x"}</Tex> is real, is <Tex>{"5"}</Tex>.</>) },
  { type: "prose", text: (<>Find the sum of the possible values of <Tex>{"a"}</Tex>.</>) },
];
const OPTIONS_Q15 = [["A", "\\sqrt{13}"], ["B", "\\sqrt{21}"], ["C", "\\sqrt{29}"], ["D", "\\sqrt{37}"], ["E", "5"], ["F", "6"]];
