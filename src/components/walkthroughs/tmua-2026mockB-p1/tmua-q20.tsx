...
const META = { questionNumber: 20, set: "B", paperNumber: "Paper 1", topicTag: "Functions / Completing the Square" };
...
const SECTIONS_Q20 = [
  { type: "prose", text: (<>The graph of <Tex>{"y = f(x)"}</Tex> is a continuous curve. The function <Tex>{"f"}</Tex> attains its maximum value of <Tex>{"2"}</Tex> at <Tex>{"x = 1"}</Tex>, and its minimum value of <Tex>{"-2"}</Tex> at <Tex>{"x = -1"}</Tex>.</>) },
  { type: "prose", text: (<>Find the difference between the maximum and minimum values of</>) },
  { type: "mathbox", tex: "(f(x))^2 + f(x)" },
];
const OPTIONS_Q20 = [["A", "\\dfrac{9}{4}"], ["B", "4"], ["C", "\\dfrac{17}{4}"], ["D", "6"], ["E", "\\dfrac{25}{4}"], ["F", "8"]];
