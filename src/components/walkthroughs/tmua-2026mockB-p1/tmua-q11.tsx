...
const META = { questionNumber: 11, set: "B", paperNumber: "Paper 1", topicTag: "Functions / Transformations" };
...
const SECTIONS_Q11 = [
  { type: "prose", text: (<>It is given that <Tex>{"f(x) = x^2 - 6x"}</Tex>.</>) },
  { type: "prose", text: (<>The curves</>) },
  { type: "mathbox", tex: "y = f(kx) \\qquad \\text{and} \\qquad y = f(x - c)" },
  { type: "prose", text: (<>have the same minimum point, where <Tex>{"k > 0"}</Tex> and <Tex>{"c > 0"}</Tex>.</>) },
  { type: "prose", text: (<>Which is a correct expression for <Tex>{"k"}</Tex> in terms of <Tex>{"c"}</Tex>?</>) },
];
const OPTIONS_Q11 = [["A", "k = \\dfrac{3}{3+c}"], ["B", "k = \\dfrac{3+c}{3}"], ["C", "k = \\dfrac{3}{c-3}"], ["D", "k = \\dfrac{c}{3}"], ["E", "k = \\dfrac{6}{6+c}"], ["F", "k = \\dfrac{6+c}{6}"]];
