...
const META = { questionNumber: 8, set: "B", paperNumber: "Paper 1", topicTag: "Trigonometry / Ambiguous Case" };
...
const SECTIONS_Q8 = [
  { type: "prose", text: (<>A triangle PQR has angle <Tex>{"QPR = 30^\\circ"}</Tex>, <Tex>{"PQ = 3\\sqrt{3}"}</Tex>, and <Tex>{"QR = 3"}</Tex>.</>) },
  { type: "prose", text: (<>There are two distinct triangles with these measurements. Let S be the one with larger area and T the one with smaller area.</>) },
  { type: "prose", text: (<>Find the ratio area of S : area of T.</>) },
];
const OPTIONS_Q8 = [["A", "1:1"], ["B", "2:1"], ["C", "3:1"], ["D", "\\sqrt{3}:1"], ["E", "2:\\sqrt{3}"], ["F", "3:2"]];
