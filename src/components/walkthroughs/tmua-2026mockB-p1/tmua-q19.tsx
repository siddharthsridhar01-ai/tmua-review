...
const META = { questionNumber: 19, set: "B", paperNumber: "Paper 1", topicTag: "Circles / Geometric Probability" };
...
const SECTIONS_Q19 = [
  { type: "prose", text: (<>Circle C1 is defined as</>) },
  { type: "mathbox", tex: "x^2 + y^2 = 16" },
  { type: "prose", text: (<>A second circle C2 has radius <Tex>{"3"}</Tex> and centre <Tex>{"(a, b)"}</Tex> where <Tex>{"-2 \\le a \\le 2"}</Tex> and <Tex>{"-2 \\le b \\le 2"}</Tex>.</>) },
  { type: "prose", text: (<>If the centre of C2 is equally likely to be located anywhere within the given range, what is the probability that C2 intersects C1?</>) },
];
const OPTIONS_Q19 = [["A", "\\dfrac{16 - \\pi}{16}"], ["B", "\\dfrac{16 + \\pi}{16}"], ["C", "\\dfrac{12 - \\pi}{12}"], ["D", "\\dfrac{4 - \\pi}{4}"], ["E", "1 - \\dfrac{\\pi}{16}"], ["F", "\\dfrac{\\pi}{16}"]];
