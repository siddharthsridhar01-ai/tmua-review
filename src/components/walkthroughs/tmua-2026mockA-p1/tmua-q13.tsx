...
const META = { questionNumber: 13, set: "A", paperNumber: "Paper 1", topicTag: "Piecewise Integration / Summation" };
...
const SECTIONS_Q13 = [
  { type: "prose", text: (<>The function <Tex>{"f"}</Tex> is such that, for every integer <Tex>{"n \\ge 0"}</Tex>,</>) },
  { type: "mathbox", tex: "\\int_n^{n+1} f(x)\\,dx = 2n + 1" },
  { type: "prose", text: (<>Evaluate the integral from <Tex>{"0"}</Tex> to <Tex>{"5"}</Tex> of <Tex>{"f(x)\\,dx"}</Tex>.</>) },
];
const OPTIONS_Q13 = [["A", "15"], ["B", "20"], ["C", "25"], ["D", "30"], ["E", "35"], ["F", "50"]];
