...
const META = { questionNumber: 19, set: "A", paperNumber: "Paper 1", topicTag: "Differential Equation / Absolute Values" };
...
const SECTIONS_Q19 = [
  { type: "prose", text: (<>The solution to the differential equation</>) },
  { type: "mathbox", tex: "\\frac{dy}{dx} = |3 - x| - |x - 1| \\quad \\text{for all } x" },
  { type: "prose", text: (<>is <Tex>{"y = f(x) + c"}</Tex>, where <Tex>{"c"}</Tex> is a constant.</>) },
  { type: "prose", text: (<>Find the value of <Tex>{"f(0) + f(2) + f(4)."}</Tex></>) },
];
const OPTIONS_Q19 = [["A", "1"], ["B", "2"], ["C", "3"], ["D", "4"], ["E", "5"], ["F", "6"]];
