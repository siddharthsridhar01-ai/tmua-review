...
const META = { questionNumber: 8, set: "A", paperNumber: "Paper 1", topicTag: "Geometric Series / Index Laws" };
...
const SECTIONS_Q8 = [
  { type: "prose", text: (<>A geometric sequence has first term <Tex>{"a"}</Tex> and common ratio <Tex>{"r"}</Tex>, where <Tex>{"a"}</Tex> and <Tex>{"r"}</Tex> are positive integers and <Tex>{"r > 1"}</Tex>.</>) },
  { type: "prose", text: (<>It is given that</>) },
  { type: "mathbox", tex: "S_{20} - S_{10} = k \\cdot S_{10}" },
  { type: "prose", text: (<>for some positive integer <Tex>{"k"}</Tex>.</>) },
  { type: "prose", text: (<>What is the smallest possible value of <Tex>{"k"}</Tex>?</>) },
];
const OPTIONS_Q8 = [["A", "2^5"], ["B", "2^{10}"], ["C", "2^{15}"], ["D", "2^{20}"], ["E", "2^{10}(2^{10} - 1)"]];
