...
const META = { questionNumber: 9, set: "A", paperNumber: "Paper 1", topicTag: "Functional Equations / Quadratic in f" };
...
const SECTIONS_Q9 = [
  { type: "prose", text: (<>This question is about pairs of functions <Tex>{"f"}</Tex> and <Tex>{"g"}</Tex> that satisfy</>) },
  { type: "mathbox", tex: "f(x) - g(x) = 3\\cos x" },
  { type: "prose", text: (<>and</>) },
  { type: "mathbox", tex: "f(x) \\cdot g(x) = \\sin^2 x" },
  { type: "prose", text: (<>for all real numbers <Tex>{"x"}</Tex>. Across all solutions for <Tex>{"f(x)"}</Tex>, what is the minimum value that <Tex>{"f(x)"}</Tex> attains for any <Tex>{"x"}</Tex>?</>) },
];
const OPTIONS_Q9 = [["A", "-4"], ["B", "-3"], ["C", "-2"], ["D", "-1"], ["E", "0"], ["F", "1"]];
