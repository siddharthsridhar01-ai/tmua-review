...
const META = { questionNumber: 16, set: "A", paperNumber: "Paper 1", topicTag: "Quartic with Trig Substitution" };
const fmt = (v, dp = 2) => { const r = Math.round(v * 100) / 100; return Number.isInteger(r) ? String(r) : parseFloat(r.toFixed(dp)).toString(); };

const opts = [
  { letter: "A", tex: "8x^4 - 6x^2 + 1 = 0", text: "8x\u2074 \u2212 6x\u00B2 + 1 = 0", ok: true, expl: "Substitute cos\u00B2\u03B8 = 1 \u2212 sin\u00B2\u03B8 into 8cos\u2074\u03B8 \u2212 10cos\u00B2\u03B8 + 3 = 0. With s = sin\u00B2\u03B8, this gives 8s\u00B2 \u2212 6s + 1 = 0, i.e. 8x\u2074 \u2212 6x\u00B2 + 1 = 0 where x = sin\u03B8." },
  { letter: "B", tex: "8x^4 + 6x^2 - 1 = 0", text: "8x\u2074 + 6x\u00B2 \u2212 1 = 0", ok: false, expl: "You might get this by a sign error when expanding (1\u2212s)\u00B2. The correct expansion of 8(1\u22122s+s\u00B2) is 8\u221216s+8s\u00B2, not 8+16s+8s\u00B2." },
  { letter: "C", tex: "8x^4 - 10x^2 + 3 = 0", text: "8x\u2074 \u2212 10x\u00B2 + 3 = 0", ok: false, expl: "This is the original equation. It works if sin\u00B2 and cos\u00B2 values happen to coincide (which they do in this case as sets), but the substitution gives a different polynomial: 8x\u2074 \u2212 6x\u00B2 + 1 = 0." },
  { letter: "D", tex: "3x^4 - 10x^2 + 8 = 0", text: "3x\u2074 \u2212 10x\u00B2 + 8 = 0", ok: false, expl: "You might get this by simply reversing the coefficients of the original equation. But that gives a different quartic with different roots." },
  { letter: "E", tex: "3x^4 - 8x^2 + 3 = 0", text: "3x\u2074 \u2212 8x\u00B2 + 3 = 0", ok: false, expl: "This doesn't arise from the correct substitution. The correct expansion of 8(1\u2212s)\u00B2 \u2212 10(1\u2212s) + 3 gives 8s\u00B2 \u2212 6s + 1, not 3s\u00B2 \u2212 8s + 3." },
  { letter: "F", tex: "3x^4 + 8x^2 - 3 = 0", text: "3x\u2074 + 8x\u00B2 \u2212 3 = 0", ok: false, expl: "This has a positive x\u00B2 coefficient which would mean the quadratic in u = x\u00B2 has a negative root, giving no real solutions for x. The correct answer has all roots real." },
];
...
const SECTIONS_Q16 = [
  { type: "prose", text: (<>The solutions to</>) },
  { type: "mathbox", tex: "8x^4 - 10x^2 + 3 = 0" },
  { type: "prose", text: (<>are <Tex>{"\\pm\\cos\\alpha"}</Tex> and <Tex>{"\\pm\\cos\\beta."}</Tex></>) },
  { type: "prose", text: (<>Which one of the following equations has solutions <Tex>{"\\pm\\sin\\alpha"}</Tex> and <Tex>{"\\pm\\sin\\beta"}</Tex>?</>) },
];
const OPTIONS_Q16 = [["A", "8x^4 - 6x^2 + 1 = 0"], ["B", "8x^4 + 6x^2 - 1 = 0"], ["C", "8x^4 - 10x^2 + 3 = 0"], ["D", "3x^4 - 10x^2 + 8 = 0"], ["E", "3x^4 - 8x^2 + 3 = 0"], ["F", "3x^4 + 8x^2 - 3 = 0"]];
  ...
  return (
    <div>
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: "18px 22px", marginBottom: 14 }}>
        ...
        <p style={{ fontSize: 15.5, lineHeight: 1.8, color: C.text, margin: "0 0 6px" }}>The solutions to</p>
        <MathBox style={{ fontSize: 17, padding: "12px 18px", margin: "8px 0" }}><Tex>{"8x^4 - 10x^2 + 3 = 0"}</Tex></MathBox>
        <p style={{ fontSize: 15.5, lineHeight: 1.8, color: C.text, margin: "6px 0 6px" }}>are <Tex>{"\\pm\\cos\\alpha"}</Tex> and <Tex>{"\\pm\\cos\\beta."}</Tex></p>
        <p style={{ fontSize: 15.5, lineHeight: 1.8, color: C.text, margin: "0 0 10px" }}>Which one of the following equations has solutions <Tex>{"\\pm\\sin\\alpha"}</Tex> and <Tex>{"\\pm\\sin\\beta"}</Tex>?</p>
      </div>
  ...
  return (
    <div>
      ...
      <InfoBox type="strategy">
        <p style={{ margin: 0 }}>The roots are <Tex>{"x = \\cos\\theta"}</Tex>, so <Tex>{"\\cos^2\\theta"}</Tex> satisfies the quadratic <Tex>{"8u^2 - 10u + 3 = 0"}</Tex>. To find the equation for <Tex>{"\\sin\\theta"}</Tex>, replace <Tex>{"\\cos^2\\theta"}</Tex> with <Tex>{"1 - \\sin^2\\theta"}</Tex> and simplify.</p>
      </InfoBox>
      <InfoBox type="insight">
        <p style={{ margin: "0 0 4px" }}>The key identity is <Tex>{"\\sin^2\\theta + \\cos^2\\theta = 1"}</Tex>, so <Tex>{"\\cos^2\\theta = 1 - \\sin^2\\theta"}</Tex>.</p>
        <p style={{ margin: 0 }}>Since <Tex>{"x = \\cos\\theta"}</Tex> satisfies <Tex>{"8x^4 - 10x^2 + 3 = 0"}</Tex>, substitute <Tex>{"x^2 = 1 - s"}</Tex> (where <Tex>{"s = \\sin^2\\theta"}</Tex>) and re-derive the polynomial in <Tex>{"s"}</Tex>.</p>
      </InfoBox>
    </div>
  );
}
...
function SolveStepContent({ revealed, setRevealed }) {
  const solveSteps = [
    { label: "APPROACH", color: C.ps,
      text: <span>The original equation holds when <Tex>{"x = \\cos\\theta"}</Tex>. Replace <Tex>{"\\cos^2\\theta"}</Tex> by <Tex>{"1 - \\sin^2\\theta"}</Tex> to find the equation that <Tex>{"\\sin\\theta"}</Tex> satisfies.</span>,
      math: (<div><Tex>{"\\cos^2\\theta = 1 - \\sin^2\\theta \\quad \\Longrightarrow \\quad \\text{let } s = \\sin^2\\theta,\\; c = 1 - s"}</Tex></div>), },
    { label: "WRITE THE ORIGINAL IN TERMS OF cos\u00B2\u03B8", color: C.calc,
      text: <span>Since <Tex>{"x = \\cos\\theta"}</Tex>, we have <Tex>{"x^2 = \\cos^2\\theta"}</Tex> and <Tex>{"x^4 = \\cos^4\\theta"}</Tex>. The equation becomes:</span>,
      math: (<><div><Tex>{"8\\cos^4\\theta - 10\\cos^2\\theta + 3 = 0"}</Tex></div><div style={{ marginTop: 4 }}><Tex>{"\\text{Let } c = \\cos^2\\theta: \\quad 8c^2 - 10c + 3 = 0"}</Tex></div></>), },
    { label: "SUBSTITUTE c = 1 \u2212 s", color: C.calc,
      text: <span>Replace <Tex>{"c"}</Tex> with <Tex>{"1 - s"}</Tex> where <Tex>{"s = \\sin^2\\theta"}</Tex>.</span>,
      math: (<><div><Tex>{"8(1-s)^2 - 10(1-s) + 3 = 0"}</Tex></div></>), },
    { label: "EXPAND AND SIMPLIFY", color: C.calc,
      text: <span>Expand <Tex>{"(1-s)^2 = 1 - 2s + s^2"}</Tex> and collect terms.</span>,
      math: (<><div><Tex>{"8(1 - 2s + s^2) - 10(1 - s) + 3 = 0"}</Tex></div><div style={{ marginTop: 4 }}><Tex>{"8 - 16s + 8s^2 - 10 + 10s + 3 = 0"}</Tex></div><div style={{ marginTop: 4 }}><Tex>{"8s^2 + (-16 + 10)s + (8 - 10 + 3) = 0"}</Tex></div><div style={{ marginTop: 4 }}><Tex>{"8s^2 - 6s + 1 = 0"}</Tex></div></>), },
    { label: "CONVERT BACK TO x", color: C.calc,
      text: <span>Since <Tex>{"s = \\sin^2\\theta = x^2"}</Tex> (where <Tex>{"x = \\sin\\theta"}</Tex>), replace <Tex>{"s"}</Tex> with <Tex>{"x^2"}</Tex>.</span>,
      math: (<><div><Tex>{"8(x^2)^2 - 6(x^2) + 1 = 0"}</Tex></div><div style={{ marginTop: 4 }}><Tex>{"\\color{#fdcb6e}{8x^4 - 6x^2 + 1 = 0}"}</Tex></div></>), },
    { label: "CONCLUSION", color: C.ok,
      text: <span>The equation with solutions <Tex>{"\\pm\\sin\\alpha"}</Tex> and <Tex>{"\\pm\\sin\\beta"}</Tex> is <Tex>{"8x^4 - 6x^2 + 1 = 0"}</Tex>.</span>,
      math: (<div><Tex>{"\\color{#55efc4}{8x^4 - 6x^2 + 1 = 0}"}</Tex></div>),
      conclusion: <span>The answer is A: <Tex>{"8x^4 - 6x^2 + 1 = 0."}</Tex></span>, },
  ];
  return (
    <div>
      <QuestionSummary />
...
function VerifyStepContent() {
  ...
  const snapPoints = [Math.PI / 6, Math.PI / 4, Math.PI / 3];
  ...
  const presets = [
    { label: "pi6", jsx: <span><Tex>{"\\theta = \\tfrac{\\pi}{6}"}</Tex></span>, val: Math.PI / 6 },
    { label: "pi4", jsx: <span><Tex>{"\\theta = \\tfrac{\\pi}{4}"}</Tex></span>, val: Math.PI / 4 },
    { label: "pi3", jsx: <span><Tex>{"\\theta = \\tfrac{\\pi}{3}"}</Tex></span>, val: Math.PI / 3 },
  ];
  ...
  return (
    <div>
      <QuestionSummary />
      <InfoBox type="hint">
        <p style={{ margin: "0 0 4px" }}>1. Slide <Tex>{"\\theta"}</Tex> to find angles where <Tex>{"\\cos\\theta"}</Tex> is a root of the original equation</p>
        <p style={{ margin: "0 0 4px" }}>2. At those angles, check that <Tex>{"\\sin\\theta"}</Tex> satisfies option A: <Tex>{"8x^4 - 6x^2 + 1 = 0"}</Tex></p>
        <p style={{ margin: 0 }}>3. Both equations should hit zero at the same <Tex>{"\\theta"}</Tex> values</p>
      </InfoBox>
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 10, padding: "10px 14px", marginBottom: 8 }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
          <span style={{ fontSize: 12, color: C.assum, fontWeight: 600 }}><Tex>{"\\theta"}</Tex></span>
          <span style={{ fontSize: 16, color: C.assum, fontWeight: 700 }}>{exactTheta ? <span><Tex>{exactTheta}</Tex><span style={{ fontFamily: mathFont, fontSize: 16, color: C.muted, marginLeft: 6 }}>{"\u2248"} {fmt(theta / Math.PI)}{"\u03C0"}</span></span> : <span style={{ fontFamily: mathFont }}>{fmt(theta / Math.PI)}{"\u03C0"}</span>}</span>
        </div>
        <input type="range" min={0.1} max={1.5} step={0.01} value={theta} onChange={e => setTheta(handleSlider(+e.target.value))} style={{ width: "100%", accentColor: C.assum, height: 6 }} />
        <div style={{ display: "flex", gap: 4, marginTop: 6 }}>
          {presets.map(pr => { const active = Math.abs(theta - pr.val) < 0.02; const prHit = active && isExact; return (<button key={pr.label} onClick={() => setTheta(pr.val)} style={{ flex: 1, padding: "6px 2px", borderRadius: 7, transition: "all 0.2s", border: `1px solid ${prHit ? C.ok : active ? C.ps : C.border}`, background: prHit ? C.ok + "15" : active ? C.ps + "15" : C.card, color: active ? (prHit ? C.ok : C.ps) : C.muted, fontSize: 12, cursor: "pointer", fontWeight: active ? 700 : 400 }}>{pr.jsx}</button>); })}
        </div>
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 8 }}>
        <div style={{ background: isOrigZero ? C.conclBg : C.card, border: `1px solid ${isOrigZero ? C.ok : C.border}`, borderRadius: 10, padding: "10px 14px", transition: "all 0.3s" }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: isOrigZero ? C.ok : C.assum, marginBottom: 6, textTransform: "uppercase", letterSpacing: 0.5 }}>Cos equation (given)</div>
          <div style={{ textAlign: "center" }}>
            <div><Tex>{`x = \\cos\\theta = ${exactCos ? exactCos + " \\approx " + fmt(cosT) : fmt(cosT)}`}</Tex></div>
            <div style={{ marginTop: 4 }}><Tex>{`\\color{${isOrigZero ? "#55efc4" : "#74b9ff"}}{8(${fmt(cosT)})^4 - 10(${fmt(cosT)})^2 + 3 = ${fmt(origLHS)}}`}</Tex></div>
          </div>
        </div>
        <div style={{ background: isNewZero ? C.conclBg : C.card, border: `1px solid ${isNewZero ? C.ok : C.border}`, borderRadius: 10, padding: "10px 14px", transition: "all 0.3s" }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: isNewZero ? C.ok : C.ps, marginBottom: 6, textTransform: "uppercase", letterSpacing: 0.5 }}>Sin equation (testing option A)</div>
          <div style={{ textAlign: "center" }}>
            <div><Tex>{`x = \\sin\\theta = ${exactSin ? exactSin + " \\approx " + fmt(sinT) : fmt(sinT)}`}</Tex></div>
            <div style={{ marginTop: 4 }}><Tex>{`\\color{${isNewZero ? "#55efc4" : "#74b9ff"}}{8(${fmt(sinT)})^4 - 6(${fmt(sinT)})^2 + 1 = ${fmt(newLHS)}}`}</Tex></div>
          </div>
        </div>
      </div>
      {isExact ? (
        <div style={{ background: C.conclBg, border: `1px solid ${C.ok}55`, borderRadius: 10, padding: "10px 14px", textAlign: "center" }}>
          <span style={{ fontSize: 14, fontWeight: 700, color: C.ok }}>At <Tex>{`\\theta = ${exactTheta}`}</Tex>: <Tex>{`\\cos\\theta = ${exactCos}`}</Tex> solves the original, <Tex>{`\\sin\\theta = ${exactSin}`}</Tex> solves <Tex>{"8x^4 - 6x^2 + 1 = 0"}</Tex>. The answer is A.</span>
        </div>
      ) : isNear ? (
        <div style={{ background: C.assumBg, border: `1px solid ${C.assum}55`, borderRadius: 10, padding: "10px 14px", textAlign: "center" }}>
          <span style={{ fontSize: 14, fontWeight: 700, color: C.assum }}>Close! Both near zero. Tap <Tex>{"\\theta = \\tfrac{\\pi}{6}"}</Tex> or <Tex>{"\\theta = \\tfrac{\\pi}{3}"}</Tex> for exact values.</span>
        </div>
      ) : (
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 10, padding: "10px 14px", textAlign: "center" }}>
          <span style={{ fontSize: 14, fontWeight: 600, color: C.muted }}>Slide <Tex>{"\\theta"}</Tex> until the cos equation hits zero, then check the sin equation.</span>
        </div>
      )}
    </div>
  );
