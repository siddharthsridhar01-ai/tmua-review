...
const META = { questionNumber: 11, paper: "Paper 1", year: "2026 Mock", topicTag: "Telescoping Series / Logarithms" };
  ...
  const graph = (() => {
    ...
    return (
      <svg viewBox={`0 0 ${pW} ${pH}`} width={pW} style={{ display: "block", width: "100%" }}>
        ...
        <foreignObject x={sx(dotPt.n) - 40} y={sy(dotPt.s) - 12 - 13} width={80} height={20}><div style={{ fontSize: 11, color: C.white, textAlign: "center", lineHeight: 1, fontWeight: 600, fontStyle: "normal", background: "rgba(15,17,23,0.7)", borderRadius: 2, padding: "0 2px", width: "fit-content", margin: "0 auto" }}><Tex>{`(${nVal}, ${fmt(dotPt.s)})`}</Tex></div></foreignObject>
        <foreignObject x={pW - pad.r - 4 - 96} y={sy(2) + 14 - 13} width={96} height={20}><div style={{ fontSize: 11, color: C.ok, textAlign: "right", lineHeight: 1, fontWeight: 600, fontStyle: "normal", background: "rgba(15,17,23,0.7)", borderRadius: 2, padding: "0 2px", width: "fit-content", marginLeft: "auto" }}><Tex>{"\\text{target} = 2"}</Tex></div></foreignObject>
        <foreignObject x={sx(50) - 72} y={pad.t + 12 - 13} width={144} height={20}><div style={{ fontSize: 11, color: C.muted, textAlign: "center", lineHeight: 1, fontWeight: 400, fontStyle: "normal", background: "rgba(15,17,23,0.7)", borderRadius: 2, padding: "0 2px", width: "fit-content", margin: "0 auto" }}><Tex>{"\\text{Partial sum } S(N)"}</Tex></div></foreignObject>
        {[10, 20, 50, 99].map(x => <foreignObject key={x} x={sx(x) - 16} y={pH - pad.b + 14 - 13} width={32} height={20}><div style={{ fontSize: 11, color: C.muted, textAlign: "center", lineHeight: 1, fontWeight: 400, fontStyle: "normal", margin: "0 auto" }}><Tex>{"x"}</Tex></div></foreignObject>)}
        {[0, 1, 2].map(y => <foreignObject key={y} x={pad.l - 8 - 32} y={sy(y) + 4 - 13} width={32} height={20}><div style={{ fontSize: 11, color: C.muted, textAlign: "right", lineHeight: 1, fontWeight: 400, fontStyle: "normal", marginLeft: "auto" }}><Tex>{"y"}</Tex></div></foreignObject>)}
      </svg>
    );
  })();
