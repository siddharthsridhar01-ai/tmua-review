  ...
  const graph = (() => {
    ...
    return (
      <svg viewBox={`0 0 ${pW} ${pH}`} width={pW} style={{ display: "block", width: "100%" }}>
        ...
        <foreignObject x={sx(xVal) - 48} y={sy(fVal) - 12 - 13} width={96} height={20}><div style={{ fontSize: 11, color: C.white, textAlign: "center", lineHeight: 1, fontWeight: 600, fontStyle: "normal", background: "rgba(15,17,23,0.7)", borderRadius: 2, padding: "0 2px", width: "fit-content", margin: "0 auto" }}><Tex>{`(${fmt(xVal / Math.PI)}{"\\u03C0"}, ${fmt(fVal)})`}</Tex></div></foreignObject>
        {/* Labels */}
        <foreignObject x={pW - pad.r - 4 - 72} y={sy(1) - 6 - 13} width={72} height={20}><div style={{ fontSize: 11, color: C.ok, textAlign: "right", lineHeight: 1, fontWeight: 600, fontStyle: "normal", background: "rgba(15,17,23,0.7)", borderRadius: 2, padding: "0 2px", width: "fit-content", marginLeft: "auto" }}><Tex>{"max = 1"}</Tex></div></foreignObject>
        <foreignObject x={pW - pad.r - 4 - 88} y={sy(1/3) + 14 - 13} width={88} height={20}><div style={{ fontSize: 11, color: C.assum, textAlign: "right", lineHeight: 1, fontWeight: 600, fontStyle: "normal", background: "rgba(15,17,23,0.7)", borderRadius: 2, padding: "0 2px", width: "fit-content", marginLeft: "auto" }}><Tex>{"min = 1/3"}</Tex></div></foreignObject>
        {/* Axis labels */}
        ...
        {[0.5, 1].map(y => <foreignObject key={y} x={pad.l - 8 - 32} y={sy(y) + 4 - 13} width={32} height={20}><div style={{ fontSize: 11, color: C.muted, textAlign: "right", lineHeight: 1, fontWeight: 400, fontStyle: "normal", marginLeft: "auto" }}><Tex>{"y"}</Tex></div></foreignObject>)}
      </svg>
    );
  })();
