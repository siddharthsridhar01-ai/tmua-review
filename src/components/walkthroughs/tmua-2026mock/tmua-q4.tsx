...
function ParabolaGraph({ compact, dotU, dotCol }) {
  ...
  return (
    <svg viewBox={`0 0 ${pW} ${pH}`} width={pW} style={{ display: "block", width: "100%" }}>
      ...
      {compact && <foreignObject x={su(3) - 32} y={sy(1) + 14 - 13} width={64} height={20}><div style={{ fontSize: 11, color: C.ok, textAlign: "center", lineHeight: 1, fontWeight: 400, fontStyle: "normal", margin: "0 auto" }}><Tex>{"(3, 1)"}</Tex></div></foreignObject>}
      {/* Moving dot with coordinates */}
      {dotVisible && <>
        <circle cx={su(dotU)} cy={sy(dotY)} r={6} fill={dotCol || C.ps} stroke={C.white} strokeWidth={1.5} />
        {!compact && <foreignObject x={su(dotU) - 40} y={sy(dotY) - 12 - 13} width={80} height={20}><div style={{ fontSize: 11, color: C.white, textAlign: "center", lineHeight: 1, fontWeight: 600, fontStyle: "normal", background: "rgba(15,17,23,0.7)", borderRadius: 2, padding: "0 2px", width: "fit-content", margin: "0 auto" }}><Tex>{`(${fmt(dotU)}, ${fmt(dotY)})`}</Tex></div></foreignObject>}
      </>}
      {/* Labels with auto-hugging backgrounds */}
      {!compact && <foreignObject x={su(3) - 64} y={sy(1) + 15 - 13} width={128} height={20}><div style={{ fontSize: 11, color: C.ok, textAlign: "center", lineHeight: 1, fontWeight: 600, fontStyle: "normal", background: "rgba(15,17,23,0.7)", borderRadius: 2, padding: "0 2px", width: "fit-content", margin: "0 auto" }}><Tex>{"u = 3, min = 1"}</Tex></div></foreignObject>}
      <foreignObject x={pW - pad.r - 30} y={sy(0) - 6 - 13} width={30} height={20}><div style={{ fontSize: 11, color: C.muted, textAlign: "right", lineHeight: 1, fontWeight: 400, fontStyle: "normal", marginLeft: "auto" }}><Tex>{"u"}</Tex></div></foreignObject>
      {!compact && <foreignObject x={su(0) + 6} y={pad.t + 10 - 13} width={112} height={20}><div style={{ fontSize: 11, color: C.muted, textAlign: "left", lineHeight: 1, fontWeight: 400, fontStyle: "normal", background: "rgba(15,17,23,0.7)", borderRadius: 2, padding: "0 2px", width: "fit-content" }}><Tex>{"(u-3)^2 + 1"}</Tex></div></foreignObject>}
      {[0, 2, 4, 6].map(u => <foreignObject key={u} x={su(u) - 16} y={pH - pad.b + 14 - 13} width={32} height={20}><div style={{ fontSize: 11, color: C.muted, textAlign: "center", lineHeight: 1, fontWeight: 400, fontStyle: "normal", margin: "0 auto" }}><Tex>{"u"}</Tex></div></foreignObject>)}
      {[2, 4, 6, 8, 10, 12].map(y => y >= yMin && y <= yMax && <foreignObject key={y} x={pad.l - 8 - 32} y={sy(y) + 4 - 13} width={32} height={20}><div style={{ fontSize: 11, color: C.muted, textAlign: "right", lineHeight: 1, fontWeight: 400, fontStyle: "normal", marginLeft: "auto" }}><Tex>{"y"}</Tex></div></foreignObject>)}
    </svg>
  );
}
