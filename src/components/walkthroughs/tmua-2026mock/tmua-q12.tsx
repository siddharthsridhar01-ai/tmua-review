  ...
  const compactGraph = (fn, { xMin, xMax, yMin, yMax, color, dots, labels, title }) => {
    ...
    return (
      <svg viewBox={`0 0 ${pW} ${pH}`} width={pW} style={{ display: "block", width: "100%" }}>
        ...
        {(labels || []).map((lb, i) => <foreignObject key={i} x={sx(lb[0])} y={sy(lb[1]) + (lb[4] || -10) - 13} width={32} height={20}><div style={{ fontSize: 11, color: lb[2] || C.white, textAlign: "left", lineHeight: 1, fontWeight: 600, fontStyle: "normal", background: "rgba(15,17,23,0.7)", borderRadius: 2, padding: "0 2px", width: "fit-content" }}><Tex>{lb[5]}</Tex></div></foreignObject>)}
        {title && <foreignObject x={pW / 2 - 16} y={pad.t - 6 - 13} width={32} height={20}><div style={{ fontSize: 11, color: C.muted, textAlign: "center", lineHeight: 1, fontWeight: 400, fontStyle: "normal", background: "rgba(15,17,23,0.7)", borderRadius: 2, padding: "0 2px", width: "fit-content", margin: "0 auto" }}><Tex>{title}</Tex></div></foreignObject>}
        {[Math.round(xMin), Math.round(xMax)].map(x => x !== 0 && <foreignObject key={x} x={sx(x) - 16} y={pH - pad.b + 14 - 13} width={32} height={20}><div style={{ fontSize: 11, color: C.muted, textAlign: "center", lineHeight: 1, fontWeight: 400, fontStyle: "normal", margin: "0 auto" }}><Tex>{"x"}</Tex></div></foreignObject>)}
      </svg>
    );
  };
    ...
    return (
      <svg viewBox={`0 0 ${pW} ${pH}`} width={pW} style={{ display: "block", width: "100%" }}>
        ...
        {qMinX >= xMin && qMinX <= xMax && quadMin >= yMin && <>
          ...
          <foreignObject x={sx(qMinX) - 40} y={sy(quadMin) - 12 - 13} width={80} height={20}><div style={{ fontSize: 11, color: C.white, textAlign: "center", lineHeight: 1, fontWeight: 600, fontStyle: "normal", background: "rgba(15,17,23,0.7)", borderRadius: 2, padding: "0 2px", width: "fit-content", margin: "0 auto" }}><Tex>{`(${fmt(qMinX)}, ${fmt(quadMin)})`}</Tex></div></foreignObject>
        </>}
        <foreignObject x={pW - pad.r - 4 - 136} y={sy(-4) - 6 - 13} width={136} height={20}><div style={{ fontSize: 11, color: C.assum, textAlign: "right", lineHeight: 1, fontWeight: 600, fontStyle: "normal", background: "rgba(15,17,23,0.7)", borderRadius: 2, padding: "0 2px", width: "fit-content", marginLeft: "auto" }}><Tex>{"target min = -4"}</Tex></div></foreignObject>
        <foreignObject x={sx(-2.5) - 56} y={sy(quartic(-2.5)) - 8 - 13} width={112} height={20}><div style={{ fontSize: 11, color: C.assum, textAlign: "center", lineHeight: 1, fontWeight: 400, fontStyle: "normal", background: "rgba(15,17,23,0.7)", borderRadius: 2, padding: "0 2px", width: "fit-content", margin: "0 auto" }}><Tex>{"x^4 - 2kx^2"}</Tex></div></foreignObject>
        <foreignObject x={sx(4) - 64} y={sy(quadratic(4)) - 8 - 13} width={128} height={20}><div style={{ fontSize: 11, color: col, textAlign: "center", lineHeight: 1, fontWeight: 600, fontStyle: "normal", background: "rgba(15,17,23,0.7)", borderRadius: 2, padding: "0 2px", width: "fit-content", margin: "0 auto" }}><Tex>{"x^2 - 2kx + 5"}</Tex></div></foreignObject>
        {[-3, -2, -1, 1, 2, 3, 4, 5].map(x => x >= xMin && x <= xMax && <foreignObject key={x} x={sx(x) - 16} y={pH - pad.b + 14 - 13} width={32} height={20}><div style={{ fontSize: 11, color: C.muted, textAlign: "center", lineHeight: 1, fontWeight: 400, fontStyle: "normal", margin: "0 auto" }}><Tex>{"x"}</Tex></div></foreignObject>)}
        {[-4, 4, 8].map(y => y >= yMin && y <= yMax && <foreignObject key={y} x={pad.l - 8 - 32} y={sy(y) + 4 - 13} width={32} height={20}><div style={{ fontSize: 11, color: C.muted, textAlign: "right", lineHeight: 1, fontWeight: 400, fontStyle: "normal", marginLeft: "auto" }}><Tex>{"y"}</Tex></div></foreignObject>)}
      </svg>
    );
  })();
