...
const META = { questionNumber: 14, paper: "Paper 1", year: "2026 Mock", topicTag: "Cubic Roots / Turning Points" };
  ...
  return (
    <div style={{ background: "#1e2030", border: `1px solid ${C.border}`, borderRadius: 10, padding: "10px 16px", marginBottom: 12, textAlign: "center" }}>
      <p style={{ margin: "0 0 4px", fontSize: 13, color: C.muted, lineHeight: 1.6 }}>
        ...
        The function <Tex>{"f(x) = x^3 - 3x + k"}</Tex> has exactly two distinct real roots. Find the sum of the possible values of <Tex>{"k."}</Tex>
      </p>
      <div style={{ display: "flex", justifyContent: "center", gap: 16, fontSize: 13, fontWeight: 600, color: C.text, flexWrap: "wrap", marginTop: 4 }}>
        {[["A","-4"],["B","-2"],["C","0"],["D","2"],["E","4"],["F","\\pm 2"]].map(([l,v]) => <span key={l}>{l}: <Tex>{v}</Tex></span>)}
      </div>
    </div>
  );
}
  ...
  const compactGraph = (kVal, label, col) => {
    ...
    return (
      <svg viewBox={`0 0 ${pW} ${pH}`} width={pW} style={{ display: "block", width: "100%" }}>
        ...
        {fNeg1 >= yMin && fNeg1 <= yMax && <foreignObject x={sx(-1) - 44} y={sy(fNeg1) - 8 - 13} width={88} height={20}><div style={{ fontSize: 11, color: C.white, textAlign: "center", lineHeight: 1, fontWeight: 600, fontStyle: "normal", background: "rgba(15,17,23,0.7)", borderRadius: 2, padding: "0 2px", width: "fit-content", margin: "0 auto" }}><Tex>{`({"\\u2212"}1, ${fmt(fNeg1)})`}</Tex></div></foreignObject>}
        {f1 >= yMin && f1 <= yMax && <foreignObject x={sx(1) - 36} y={sy(f1) + 16 - 13} width={72} height={20}><div style={{ fontSize: 11, color: C.white, textAlign: "center", lineHeight: 1, fontWeight: 600, fontStyle: "normal", background: "rgba(15,17,23,0.7)", borderRadius: 2, padding: "0 2px", width: "fit-content", margin: "0 auto" }}><Tex>{`(1, ${fmt(f1)})`}</Tex></div></foreignObject>}
        <foreignObject x={pW / 2 - 16} y={pad.t - 6 - 13} width={32} height={20}><div style={{ fontSize: 11, color: C.muted, textAlign: "center", lineHeight: 1, fontWeight: 400, fontStyle: "normal", background: "rgba(15,17,23,0.7)", borderRadius: 2, padding: "0 2px", width: "fit-content", margin: "0 auto" }}><Tex>{label}</Tex></div></foreignObject>
      </svg>
    );
  };
  ...
  const graph = (() => {
    ...
    return (
      <svg viewBox={`0 0 ${pW} ${pH}`} width={pW} style={{ display: "block", width: "100%" }}>
        ...
        {fNeg1 >= yMin && fNeg1 <= yMax && <foreignObject x={sx(-1) - 44} y={sy(fNeg1) - 12 - 13} width={88} height={20}><div style={{ fontSize: 11, color: C.white, textAlign: "center", lineHeight: 1, fontWeight: 600, fontStyle: "normal", background: "rgba(15,17,23,0.7)", borderRadius: 2, padding: "0 2px", width: "fit-content", margin: "0 auto" }}><Tex>{`({"\\u2212"}1, ${fmt(fNeg1)})`}</Tex></div></foreignObject>}
        {f1 >= yMin && f1 <= yMax && <foreignObject x={sx(1) - 36} y={sy(f1) + 18 - 13} width={72} height={20}><div style={{ fontSize: 11, color: C.white, textAlign: "center", lineHeight: 1, fontWeight: 600, fontStyle: "normal", background: "rgba(15,17,23,0.7)", borderRadius: 2, padding: "0 2px", width: "fit-content", margin: "0 auto" }}><Tex>{`(1, ${fmt(f1)})`}</Tex></div></foreignObject>}
        <foreignObject x={pW / 2 - 80} y={pad.t + 12 - 13} width={160} height={20}><div style={{ fontSize: 11, color: C.muted, textAlign: "center", lineHeight: 1, fontWeight: 400, fontStyle: "normal", background: "rgba(15,17,23,0.7)", borderRadius: 2, padding: "0 2px", width: "fit-content", margin: "0 auto" }}><Tex>{"f(x) = x^3 - 3x + k"}</Tex></div></foreignObject>
        ...
        {[-4, -2, 2, 4].map(y => y >= yMin && y <= yMax && <foreignObject key={y} x={pad.l - 8 - 32} y={sy(y) + 4 - 13} width={32} height={20}><div style={{ fontSize: 11, color: C.muted, textAlign: "right", lineHeight: 1, fontWeight: 400, fontStyle: "normal", marginLeft: "auto" }}><Tex>{"y"}</Tex></div></foreignObject>)}
      </svg>
    );
  })();
