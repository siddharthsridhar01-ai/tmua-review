...
function SolveStepContent({ revealed, setRevealed }) {
  const solveSteps = [
    ...
    { label: "CONDITION 2: a < b (OPPOSITE < ADJACENT)", color: C.calc, text: <span>The opposite side must be shorter than the adjacent side, otherwise the arc centred at B with radius <Tex>{"a"}</Tex> can only cross the ray from A once (or not at all), giving at most one triangle.</span>, math: (<><div><Tex>{"x - 1 < -x^2 + 6x - 5"}</Tex></div><div style={{ marginTop: 4 }}><Tex>{"x^2 - 5x + 4 < 0"}</Tex></div><div style={{ marginTop: 4 }}><Tex>{"(x - 1)(x - 4) < 0 \\;\\Rightarrow\\; 1 < x < 4"}</Tex></div></>) },
    ...
    { label: "INTERSECT ALL CONDITIONS", color: C.calc, text: <span>Combine: <Tex>{"1 < x < 5"}</Tex> (positivity), <Tex>{"1 < x < 4"}</Tex> (opposite {"<"} adjacent), and <Tex>{"x > 3"}</Tex> (above height).</span>, math: (<><div><Tex>{"(1 < x < 5) \\;\\cap\\; (1 < x < 4) \\;\\cap\\; (x > 3)"}</Tex></div><div style={{ marginTop: 4 }}><Tex>{"= 3 < x < 4"}</Tex></div></>) },
