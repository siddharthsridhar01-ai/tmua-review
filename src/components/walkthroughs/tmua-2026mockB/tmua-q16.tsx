...
function QuestionSummary() { return (<div style={{ background: "#1e2030", border: `1px solid ${C.border}`, borderRadius: 10, padding: "10px 16px", marginBottom: 12, textAlign: "center" }}><p style={{ margin: "0 0 4px", fontSize: 13, color: C.muted, lineHeight: 1.6 }}><span style={{ fontWeight: 700, color: C.muted, letterSpacing: 0.5, marginRight: 6 }}>Q16</span>A right-angled triangle has vertices at <Tex>{"(0, 1)"}</Tex>, <Tex>{"(8, 5)"}</Tex> and <Tex>{"(2, k)"}</Tex>. Find the sum of all possible values of <Tex>{"k"}</Tex>.</p><div style={{ display: "flex", justifyContent: "center", gap: 6, fontSize: 13, fontWeight: 600, color: C.text, flexWrap: "wrap", marginTop: 4 }}>{[["A","16"],["B","18"],["C","20"],["D","22"],["E","24"],["F","28"]].map(([l,v]) => <span key={l}>{l}: <Tex>{v}</Tex></span>)}</div></div>); }
