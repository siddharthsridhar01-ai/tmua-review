...
const META = { questionNumber: 8, paper: "Paper 1", year: "2026 Mock Set B", topicTag: "Trigonometry / Ambiguous Case" };
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
        <div style={{ background: C.card, border: `1px solid ${C.ps}44`, borderRadius: 10, padding: "12px 14px" }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: C.ps, marginBottom: 6 }}>Triangle S (larger)</div>
          <div style={{ fontSize: 13, color: C.text, lineHeight: 1.8 }}>
            <div><Tex>{"R = 60^\\circ, \\quad Q = 90^\\circ"}</Tex></div>
            <div><Tex>{"\\sin(\\angle Q) = 1"}</Tex></div>
            <div style={{ marginTop: 4 }}>Area = <Tex>{"\\tfrac{9\\sqrt{3}}{2}"}</Tex> <span style={{ color: C.muted }}>{"\u2248"} <Tex>{fmt(areaS)}</Tex></span></div>
          </div>
        </div>
        <div style={{ background: C.card, border: `1px solid ${C.assum}44`, borderRadius: 10, padding: "12px 14px" }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: C.assum, marginBottom: 6 }}>Triangle T (smaller)</div>
          <div style={{ fontSize: 13, color: C.text, lineHeight: 1.8 }}>
            <div><Tex>{"R = 120^\\circ, \\quad Q = 30^\\circ"}</Tex></div>
            <div><Tex>{"\\sin(\\angle Q) = \\tfrac{1}{2}"}</Tex></div>
            <div style={{ marginTop: 4 }}>Area = <Tex>{"\\tfrac{9\\sqrt{3}}{4}"}</Tex> <span style={{ color: C.muted }}>{"\u2248"} <Tex>{fmt(areaT)}</Tex></span></div>
          </div>
        </div>
      </div>
      {/* Ratio panels */}
