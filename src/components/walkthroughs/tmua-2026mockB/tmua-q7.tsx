...
const META = { questionNumber: 7, paper: "Paper 1", year: "2026 Mock Set B", topicTag: "Exponentials / Substitution" };
  ...
  return (
    <div style={{ background: "#1e2030", border: `1px solid ${C.border}`, borderRadius: 10, padding: "10px 16px", marginBottom: 12, textAlign: "center" }}>
      <p style={{ margin: "0 0 4px", fontSize: 13, color: C.muted, lineHeight: 1.6 }}>
        ...
        Find the largest value of <Tex>{"x"}</Tex> such that <Tex>{"4^x - 5 \\cdot 2^x + 6 = 0"}</Tex>.
      </p>
      <div style={{ display: "flex", justifyContent: "center", gap: 16, fontSize: 13, fontWeight: 600, color: C.text, flexWrap: "wrap", marginTop: 4 }}>
        {[["A","1"],["B","\\log_2 3"],["C","\\log_2 5"],["D","2"],["E","\\log_2 6"],["F","3"]].map(([l,v]) => <span key={l}>{l}: <Tex>{v}</Tex></span>)}
      </div>
    </div>
  );
}
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6, marginBottom: 8 }}>
        <div style={{ background: C.card, border: `1px solid ${C.calc}44`, borderRadius: 8, padding: "7px 4px", textAlign: "center" }}>
          <div style={{ fontSize: 11, color: C.calc, fontWeight: 700, marginBottom: 2 }}><Tex>{"x"}</Tex></div>
          <div style={{ fontSize: 20, fontWeight: 700, color: C.calc }}>{snappedExact ? <Tex>{snappedExact.tex}</Tex> : <Tex>{fmt(xVal)}</Tex>}</div>
        </div>
        <div style={{ background: C.card, border: `1px solid ${C.ps}44`, borderRadius: 8, padding: "7px 4px", textAlign: "center" }}>
          <div style={{ fontSize: 11, color: C.ps, fontWeight: 700, marginBottom: 2 }}><Tex>{"u = 2^x"}</Tex></div>
          <div style={{ fontSize: 20, fontWeight: 700, color: C.ps }}><Tex>{fmt(uVal)}</Tex></div>
        </div>
        <div style={{ background: isExact ? C.conclBg : isNear ? C.assumBg : C.card, border: `1px solid ${col}44`, borderRadius: 8, padding: "7px 4px", textAlign: "center", transition: "all 0.3s" }}>
          <div style={{ fontSize: 11, color: col, fontWeight: 700, marginBottom: 2 }}>LHS</div>
          <div style={{ fontSize: 20, fontWeight: 700, color: col }}><Tex>{fmt(lhs)}</Tex></div>
        </div>
      </div>
