...
function QuestionSummary() { return (<div style={{ background: "#1e2030", border: `1px solid ${C.border}`, borderRadius: 10, padding: "10px 16px", marginBottom: 12, textAlign: "center" }}><p style={{ margin: "0 0 4px", fontSize: 13, color: C.muted, lineHeight: 1.6 }}><span style={{ fontWeight: 700, color: C.muted, letterSpacing: 0.5, marginRight: 6 }}>Q19</span>Circle C1 is <Tex>{"x^2 + y^2 = 16"}</Tex>. Circle C2 has radius <Tex>{"3"}</Tex> and centre <Tex>{"(a, b)"}</Tex> where <Tex>{"-2 \\le a \\le 2"}</Tex> and <Tex>{"-2 \\le b \\le 2"}</Tex>. What is the probability that C2 intersects C1?</p><div style={{ display: "flex", justifyContent: "center", gap: 6, fontSize: 13, fontWeight: 600, color: C.text, flexWrap: "wrap", marginTop: 4 }}>{[["A","\\frac{16-\\pi}{16}"],["B","\\frac{16+\\pi}{16}"],["C","\\frac{12-\\pi}{12}"],["D","\\frac{4-\\pi}{4}"],["E","1-\\frac{\\pi}{16}"],["F","\\frac{\\pi}{16}"]].map(([l,v]) => <span key={l}>{l}: <Tex>{v}</Tex></span>)}</div></div>); }
