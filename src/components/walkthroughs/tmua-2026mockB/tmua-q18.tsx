...
function QuestionSummary() { return (<div style={{ background: "#1e2030", border: `1px solid ${C.border}`, borderRadius: 10, padding: "10px 16px", marginBottom: 12, textAlign: "center" }}><p style={{ margin: "0 0 4px", fontSize: 13, color: C.muted, lineHeight: 1.6 }}><span style={{ fontWeight: 700, color: C.muted, letterSpacing: 0.5, marginRight: 6 }}>Q18</span>The curve <Tex>{"y = (x-1)^2 + 2"}</Tex> is rotated <Tex>{"90^\\circ"}</Tex> clockwise about the point <Tex>{"(3, 2)"}</Tex> to give the curve C. What is the equation of C?</p><div style={{ display: "flex", justifyContent: "center", gap: 6, fontSize: 13, fontWeight: 600, color: C.text, flexWrap: "wrap", marginTop: 4 }}>{[["A","x=(4-y)^2+3"],["B","x=-(4-y)^2+3"],["C","y=(x-4)^2+3"],["D","y=-(x-3)^2+4"],["E","x=(y-4)^2+3"],["F","y=-(x-4)^2-3"]].map(([l,v]) => <span key={l}>{l}: <Tex>{v}</Tex></span>)}</div></div>); }
